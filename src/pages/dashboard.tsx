...
function DashboardPage() {
  ...
  const vehicleManagerStat = useSWR<PaginatedInterface<VehicleManagerInterface>>(
    () => `/vehicle-managers`,
    () => getVehicleManagers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: vehicleManagerStat?.data?.totalCount,
      isLoading: vehicleManagerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Vehicles',
